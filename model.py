...
from voxel.augmentation import apply_se3_augmentation_with_camera_pose_final, apply_se3_augmentation_with_camera_pose
from helpers.clip.core.clip import build_model, load_clip
import PIL.Image as Image
import transformers
from helpers.optim.lamb import Lamb
from torch.nn.parallel import DistributedDataParallel as DDP
from agents.gengaussian_bc.neural_rendering import NeuralRenderer
from agents.gengaussian_bc.utils import visualize_pcd
...
class QFunction(nn.Module):
    ...
    def choose_highest_action_(self, q_trans, q_rot_grip, q_collision, bounds, res):
        coords = self._argmax_3d(q_trans)
        coords = coords.int()  # torch.Size([1, 3])
        position = bounds[:, :3] + res * coords + res / 2
        ...
        if q_rot_grip is not None:
            ...
            rot_and_grip_indicies = torch.cat(
                [q_rot[:, 0:1].argmax(-1),
                 ...
                 q_rot_grip[:, -2:].argmax(-1, keepdim=True)], -1)
            ignore_collision = q_collision[:, -2:].argmax(-1, keepdim=True)
        return coords, position, rot_and_grip_indicies, ignore_collision


    def forward(self, rgb_pcd, depth, proprio, pcd, camera_extrinsics, camera_intrinsics, lang_goal_emb,
                lang_token_embs, bounds=None, prev_bounds=None, prev_layer_voxel_grid=None,
                use_neural_rendering=False, nerf_target_rgb=None, nerf_target_depth=None,
                nerf_target_pose=None, nerf_target_camera_intrinsic=None, lang_goal=None,
                pcd_next=None, nerf_next_target_rgb=None, nerf_next_target_pose=None, nerf_next_target_depth=None,
                nerf_next_target_camera_intrinsic=None, gt_embed=None, step=None, action=None):
        '''
        ...
        # pcd_next_flat = torch.cat(
        #     [p.permute(0, 2, 3, 1).reshape(b, -1, 3) for p in pcd_next], 1) if pcd_next is not None else None
        if pcd_next is not None:
            pcd_next_flat = [p.permute(0, 2, 3, 1).reshape(b, -1, 3) for p in pcd_next]
        else:
            pcd_next_flat = None
        ...
        voxel_data = None
        voxel_data = (self._voxelizer.coords_to_bounding_voxel_grid(
            pcd_flat, coord_features=flat_imag_features, coord_bounds=bounds, return_density=True,
            include_per_voxel_coor=True))
        voxel_grid, voxel_density, coord_positions, indices_25, indices_50, linear_indices_20 = voxel_data
        # voxel_data = None
       
        # swap to channels fist
        voxel_grid = voxel_grid.permute(0, 4, 1, 2,
                                        3).detach()  
        ...
        # forward pass
        q_trans, q_rot_and_grip, q_ignore_collisions, voxel_grid_feature, multi_scale_voxel_list, lang_embedd = (
            self._qnet(
                voxel_grid,  # [1,10,100^3]
                proprio,  # [1,4]
        ...
        if use_neural_rendering:  # train default: True; eval default: False
            ...
            if nerf_target_rgb is not None:
                ...
                depth_0 = depth[0]
                pcd_0 = pcd[0]
                if pcd_next_flat is not None:
                    if len(pcd_next_flat[0].shape) == 3:
                        pcd_next_flat_0 = pcd_next_flat[0][0]
                    else:
                        pcd_next_flat_0 = pcd_next_flat[0]
                else:
                    pcd_next_flat_0 = None

                # TODO:use front and wrist camera for gaussian model

                # render loss
                rendering_loss_dict, _ = self._neural_renderer(
                    rgb=rgb_0,
                    pcd=pcd_0,
                    pcd_next=pcd_next_flat_0,
                    depth=depth_0,
    ...
    @torch.no_grad()
    def render(self, rgb_pcd, proprio, pcd, depth=None, camera_extrinsics=None, camera_intrinsics=None,
               lang_goal_emb=None, lang_token_embs=None, lang_goal=None, bounds=None, prev_layer_bounds=None,
               prev_layer_voxel_grid=None, tgt_pose=None, tgt_intrinsic=None, nerf_target_rgb=None,
               nerf_target_depth=None, nerf_next_target_pose=None, nerf_next_target_camera_intrinsic=None,
               nerf_next_target_rgb=None, nerf_next_target_depth=None, action=None, step=None,
               ):
        """
        ...
        # swap to channels fist
        voxel_grid = voxel_grid.permute(0, 4, 1, 2,
                                        3).detach()  # Bx10x100x100x100 10(3rgb + 3pcd + 3density<index/100 + 1occupy +)
        ...
        _, ret_dict = self._neural_renderer(
            ...
            lang_goal=lang_goal,
            next_gt_rgb=nerf_next_target_rgb,
            next_gt_depth=nerf_next_target_depth,
            next_gt_pose=nerf_next_target_pose,
            next_gt_intrinsic=nerf_next_target_camera_intrinsic,
            step=step,
            action=action,
        ...
        return ret_dict.render_novel, ret_dict.next_render_novel, ret_dict.render_embed, ret_dict.gt_embed, ret_dict.next_render_embed, ret_dict.next_gt_embed, ret_dict.render_depth, ret_dict.next_render_depth
...
class QAttentionPerActBCAgent(Agent):
    ...
    def __init__(self,
                 ...
                 include_low_dim_state_11: bool = False,
        ...
        self._include_low_dim_state_11 = include_low_dim_state_11
    ...
    def build(self, training: bool, device: torch.device = None, use_ddp=True, fabric: Fabric = None):
        ...
        self._voxelizer = VoxelGrid(
            coord_bounds=self._coordinate_bounds.cpu() if isinstance(self._coordinate_bounds,
                                                                     torch.Tensor) else self._coordinate_bounds,
        ...
        if self._training:
            ...
            self.language_model = create_language_model(self.cfg.language_model, device=device)
    ...
    def update(self, step: int, replay_sample: dict, fabric: Fabric) -> dict:
        ...
        action_gripper_pose = replay_sample['gripper_pose']  # [bs, 7]
        ...
        if nerf_multi_view_rgb_path is None or nerf_multi_view_rgb_path[0, 0] is None:
            ...
            global choice_NUM
            view_dix = choice_NUM % num_view_by_user
            choice_NUM += 1
            ...
            for i in range(bs):
                nerf_target_rgbs.append(parse_img_file(nerf_multi_view_rgb_path[i],
                                                       mask_gt_rgb=self._mask_gt_rgb))  # , session=self._rembg_session))    # FIXME: file_path 'NoneType' object has no attribute 'read'
                nerf_target_depths.append(parse_depth_file(nerf_multi_view_depth_path[i]))
                nerf_target_camera_extrinsic, nerf_target_camera_intrinsic, nerf_target_focal = parse_camera_file(
                    nerf_multi_view_camera_path[i])
                ...
                nerf_next_target_rgbs.append(parse_img_file(nerf_next_multi_view_rgb_path[i],
                                                            mask_gt_rgb=self._mask_gt_rgb))  # , session=self._rembg_session))    # FIXME: file_path 'NoneType' object has no attribute 'read'
                nerf_next_target_depths.append(parse_depth_file(nerf_next_multi_view_depth_path[i]))
                nerf_next_target_camera_extrinsic, nerf_next_target_camera_intrinsic, nerf_next_target_focal = parse_camera_file(
                    nerf_next_multi_view_camera_path[i])
            ...
            nerf_target_depth = torch.from_numpy(np.stack(nerf_target_depths)).float().to(
                device)  # [bs, H, W, 1], no normalization
            ...
            nerf_next_target_rgb = torch.from_numpy(np.stack(nerf_next_target_rgbs)).float().to(
                device)  # [bs, H, W, 3], [0,1]
            nerf_next_target_depth = torch.from_numpy(np.stack(nerf_next_target_depths)).float().to(
                device)  # [bs, H, W, 1], no normalization
            nerf_next_target_camera_extrinsic = torch.from_numpy(
                np.stack(nerf_next_target_camera_extrinsics)).float().to(device)
            nerf_next_target_camera_intrinsic = torch.from_numpy(
                np.stack(nerf_next_target_camera_intrinsics)).float().to(device)

            pcd_next = []
            for n in self._camera_names:  # default: [front,left_shoulder,right_shoulder,wrist] or [front]
                pcd_ = replay_sample['%s_point_cloud' % n]
                pcd_next.append(pcd_)
        ...
        proprio = None
        proprio = replay_sample['low_dim_state'][:, 7:]
        curr_gripper_pose = replay_sample['low_dim_state'][:, :7]
        if self._include_low_dim_state_11:
            proprio = torch.cat([curr_gripper_pose, proprio], dim=-1)

        # SE(3) augmentation of point clouds and actions
        if self._transform_augmentation:  # default: True
            curr_gripper_pose, action_gripper_pose, action_trans, action_rot_grip, pcd, extrinsics = (
                apply_se3_augmentation_with_camera_pose_final(pcd,
                                                              extrinsics,
                                                              action_gripper_pose,
                                                              action_trans,
                                                              action_rot_grip,
        ...
        coords, position, rot_and_grip_indicies, ignore_collision_indicies = (
            self._q.choose_highest_action_(q_trans, q_rot_grip, q_collision, bounds, res))
        ...
        position_mse_loss = F.mse_loss(position, action_gripper_pose[:, :3])
        ...
        if self.use_neural_rendering:  # eval default: False; train default: True
            ...
            loss_depth_item = rendering_loss_dict['loss_depth']
            loss_dyna_rgb_item = rendering_loss_dict['loss_dyna_rgb']
            loss_dyna_embed_item = rendering_loss_dict['loss_dyna_embed']
            loss_dyna_depth_item = rendering_loss_dict['loss_dyna_depth']
            psnr = rendering_loss_dict['psnr']
            dyna_psnr = rendering_loss_dict['dyna_psnr']

            if step % 10 == 0 and rank == 0:
                print()
                cprint(f'total L: {total_loss.item():.6f} | \
                    L_BC: {combined_losses.item():.6f}  | \
                    L_trans: {q_trans_loss.item():.6f} | \
                    L_pos_0: {position_mse_loss.item():.6f} | \
                    L_rot: {q_rot_loss.item():.6f} | \
                    L_grip: {q_grip_loss.item():.6f}  | \
                    L_col: {q_collision_loss.item():.6f}', 'green')
                print()
                cprint(f"L_nerf: {rendering_loss_dict['loss'].item():.6f}  | \
                    L_rgb: {loss_rgb_item:.6f}  | \
                    L_embed: {loss_embed_item:.6f}  | \
                    L_depth: {loss_depth_item:.6f}  | \
                    L_dyna_rgb: {loss_dyna_rgb_item:.6f}  | \
                    L_dyna_embed: {loss_dyna_embed_item:.6f}  | \
                    L_dyna_depth: {loss_dyna_depth_item:.6f}  | \
                    psnr_0: {psnr:.6f}  | \
                    psnr_dyna: {dyna_psnr:.6f}", 'green')
                print()
                ...
                if self.cfg.use_wandb:
                    wandb.log({
                        ...
                        'train/position_mse': position_mse_loss.item(),
                        'train/psnr': psnr,
                        'train/dyna_psnr': dyna_psnr,
                        'train/rgb_loss': loss_rgb_item,
                        'train/embed_loss': loss_embed_item,
                        'train/depth_loss': loss_depth_item,
                        'train/dyna_rgb_loss': loss_dyna_rgb_item,
                        'train/dyna_embed_loss': loss_dyna_embed_item,
                        'train/dyna_depth_loss': loss_dyna_depth_item,
                    }, step=step)
        ...
        if to_render:
            rgb_render, next_rgb_render, embed_render, gt_embed_render, next_embed_render, next_gt_embed_render, depth_render, next_depth_render = self._q.render(
                rgb_pcd=obs,
                proprio=proprio,
                pcd=pcd,
                depth=depth,
                # todo: add those two parameters
            ...
            if next_rgb_render is not None:
                ...
                next_depth_gt = nerf_next_target_depth[0]
                next_rgb_render = next_rgb_render[0]
                next_depth_render = next_depth_render[0]
                psnr_dyna = PSNR_torch(next_rgb_render, next_rgb_gt)

            os.makedirs('recon', exist_ok=True)
            import matplotlib.pyplot as plt
            # plot three images in two row with subplots:
            # src, tgt_rgb, pred_rgb, tgt_embed, pred_embed, tgt_depth, pred_depth
            # next_tgt_rgb, next_pred_rgb, next_tgt_embed, next_pred_embed, next_tgt_depth, next_pred_depth
            rgb_src = obs[0][0].squeeze(0).permute(1, 2, 0) / 2 + 0.5
            fig, axs = plt.subplots(2, 7, figsize=(23, 7))
            # src
            axs[0, 0].imshow(rgb_src.cpu().numpy())
            axs[0, 0].title.set_text('src_rgb')
            # tgt
            axs[0, 1].imshow(rgb_gt.cpu().numpy())
            axs[0, 1].title.set_text('tgt_rgb')
            # pred rgb
            axs[0, 2].imshow(rgb_render.cpu().numpy())
            axs[0, 2].title.set_text('pred_rgb:psnr={:.2f}'.format(psnr))
            # gt embed
            if gt_embed_render is not None:
                ...
                axs[0, 3].imshow(gt_embed_render)
                axs[0, 3].title.set_text('tgt_embed')
                # pred embed
                # embed_render = visualize_feature_map_by_clustering(embed_render.permute(0,3,1,2), num_cluster=4)
                embed_render = visualize_feature_map_by_normalization(
                    embed_render.permute(0, 3, 1, 2))  # range from -1 to 1
                axs[0, 4].imshow(embed_render)
                axs[0, 4].title.set_text('pred_embed')
            if depth_render is not None:
                # gt depth
                axs[0, 5].imshow(depth_gt.cpu().numpy())
                axs[0, 5].title.set_text('tgt_depth')
                # pred depth
                axs[0, 6].imshow(depth_render.cpu().numpy())
                axs[0, 6].title.set_text('pred_depth')
            if next_rgb_render is not None:
                # gt next rgb frame
                axs[1, 1].imshow(next_rgb_gt.cpu().numpy())
                axs[1, 1].title.set_text('next_tgt_rgb')
                # Ours
                axs[1, 2].imshow(next_rgb_render.cpu().numpy())
                axs[1, 2].title.set_text('next_pred_rgb:psnr={:.2f}'.format(psnr_dyna))
            if next_embed_render is not None:
                next_gt_embed_render = visualize_feature_map_by_normalization(
                    next_gt_embed_render)  # range from -1 to 1
                axs[1, 3].imshow(next_gt_embed_render)
                axs[1, 3].title.set_text('next_gt_embed')
                next_embed_render = visualize_feature_map_by_normalization(
                    next_embed_render.permute(0, 3, 1, 2))  # range from -1 to 1
                axs[1, 4].imshow(next_embed_render)
                axs[1, 4].title.set_text('next_pred_embed')
            if next_depth_render is not None:
                axs[1, 5].imshow(next_depth_gt.cpu().numpy())
                axs[1, 5].title.set_text('next_tgt_depth')
                axs[1, 6].imshow(next_depth_render.cpu().numpy())
                axs[1, 6].title.set_text('next_pred_depth')
            # remove axis
            for axs_ in axs:
                for ax in axs_:
                    ax.axis('off')
    ...
    def act(self, step: int, observation: dict,
        ...
        proprio = None
        proprio = observation['low_dim_state'][:, :, 7:]
        if self._include_low_dim_state_11:
    ...
    def farthest_point_sample(self, xyz, pointnumber=9999):
        device = xyz.device
        N, C = xyz.shape
        ...
        results = xyz[centroids.detach().cpu().numpy().astype(int)]
        return results

    def chamfer_distance(self, point_set1, point_set2):
        batch_size, num_points1, dim = point_set1.size()
        _, num_points2, _ = point_set2.size()

        # Calculate the square distance between points
        point_set1_expand = point_set1.unsqueeze(2).expand(batch_size, num_points1, num_points2, dim)
        point_set2_expand = point_set2.unsqueeze(1).expand(batch_size, num_points1, num_points2, dim)
        distances = torch.norm(point_set1_expand - point_set2_expand, dim=3)

        # Calculate the closest distance from each point to the cloud at another point
        min_distances1, _ = torch.min(distances, dim=2)
        min_distances2, _ = torch.min(distances, dim=1)

        # Calculating Chamfer Distance
        chamfer_dist = torch.mean(min_distances1, dim=1) + torch.mean(min_distances2, dim=1)
